...
object PFBSchemas {
  val linkSchema: Schema = SchemaBuilder.record("Link")
    .fields()
    .name("multiplicity")
    .`type`().enumeration("Multiplicity")
  ...
  val propertySchema: Schema = SchemaBuilder.record("Property")
    .fields()
    .requiredString("name")
    .requiredString("ontology_reference")
    .name("values").`type`().map().values().`type`("string").noDefault()
    .endRecord()

  val nodeSchema: Schema = SchemaBuilder.record("Node")
    .fields()
    .requiredString("name")
    .requiredString("ontology_reference")
    .name("values").`type`().map().values().`type`("string").noDefault()
    .name("links").`type`().array().items(linkSchema).noDefault()
    .name("properties").`type`().array().items(propertySchema).noDefault()
    .endRecord()

  val metadataSchema: Schema = SchemaBuilder.record("Metadata")
    .fields()
    .name("nodes")
      .`type`().array()
      .items()
    ...
    .name("misc")
      .`type`().map().values().`type`("string").noDefault()
    .endRecord()
