  ...
  def writePFB(reader: CSVReader, properties: Map[String, JValue], pfbFilename: File): Unit = {
    ...
    headerRow foreach { colName =>
      ...
      props foreach {
        ...
        case value =>
          new RuntimeException(s"Expected JObject but obtained $value for property $colName")
      }
    }
    ...
    dataFileWriter.create(
      schema,
      pfbFilename
    ) // I ran into some problems when writing this to an existing BufferedWriter,
    ...
    dataWithHeaders.zipWithIndex.foreach({
      case (row, index) =>
        ...
        row.keys.map({ colName =>
          ...
          scribe.debug(s"Output(${mapFieldNameToPFB(colName)}, ${if (mappedValue == null) "<null>"
          else mappedValue})")
