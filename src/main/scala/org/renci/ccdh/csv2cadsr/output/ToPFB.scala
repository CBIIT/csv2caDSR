...
object ToPFB {
  ...
  /**
    ...
    */
  def mapFieldNameToPFB(fieldName: String): String =
    fieldName
      .map({
        case '.'                          => "."
  ...
  def writePFB(reader: CSVReader, properties: Map[String, JValue], pfbFilename: File): Unit = {
    ...
    headerRow foreach { rowName =>
      properties.get(rowName) foreach {
        case property: JObject =>
          val fieldType: String = property.values
            .get("type")
            .flatMap({
            ...
            case Some("number") =>
              "string" // We'll store it as a string and let it be reparsed (hopefully into BigDecimal) at the other end.
            case Some(str) => str
            case None      => "string"
          }
          ...
          if (property.values.contains("permissibleValues")) {
                  ...
                  .map({
                    ...
                    case unk =>
                      throw new RuntimeException(
                        s"Cannot read value in array in enumValues in property '$rowName': $unk"
                      )
                  })
                  .filter(_.nonEmpty)
                  .map(mapFieldNameToPFB)
              case unk =>
                throw new RuntimeException(
                  s"Cannot read value in enumValues in property '$rowName': $unk"
                )
            }
            ...
            if (permissibleValues.isEmpty) {
              ...
              fieldsBuilder = fieldsBuilder
                ...
                .`type`(
                  Schema
                    .createUnion(Schema.create(Schema.Type.NULL), Schema.create(Schema.Type.STRING))
                )
                .noDefault()
            } else {
    ...
    metadataNode.put(
      "properties",
      CollectionConverters.asJava(headerRow.flatMap({ rowName =>
        properties.get(rowName) map {
          case property: JObject =>
                ...
                prop.put(
                  ...
                  CollectionConverters.asJava(
                    Map(
                      ...
                      "term_url" -> s"https://cdebrowser.nci.nih.gov/cdebrowserClient/cdeBrowser.html#/search?publicId=${cdeId}&version=${cdeIdVersion
                        .getOrElse("1.0")}"
                    )
                  )
                )
            ...
            prop
          case value: JValue =>
            throw new RuntimeException(s"Unable to interpret property '$value': not a JObject")
        }
      }))
    )
    ...
    dataWithHeaders.zipWithIndex.foreach({
      case (row, index) =>
        ...
        row.keys.map({ colName =>
          ...
          val mappedValue: Any =
            if (value == "") null
            else if (!harmonizationMappings.contains(colName)) {
              fieldType match {
              ...
              val mappedValues: Seq[String] = harmonizationMappings.get(colName) match {
                case Some(JArray(list)) =>
                  list flatMap {
                    case JObject(mappingValues) => {
                      ...
                      if (inputValue.getOrElse("").equals(JString(value))) {
                        mapping.get("caDSRValue") match {
                          case Some(JString("")) => {
                            throw new RuntimeException(
                              s"Could not map verbatim value '$inputValue' in $colName: no mapping in input JSON file"
                            )
                          }
                          case Some(JString(str)) => {
                            scribe.info(
                              s"Mapping verbatim value '$inputValue' in $colName to: $str"
                            )
                            Some(mapFieldNameToPFB(str))
                          }
                          case unk =>
                            throw new RuntimeException(
                              s"Unexpected value in caDSRValue field of harmonization mappings of ${colName}: ${unk}"
                            )
                        }
                      } else {
                    ...
                    case unk =>
                      throw new RuntimeException(
                        s"Unexpected value in harmonization mappings of ${colName}: ${unk}"
                      )
                  }
                case unk =>
                  throw new RuntimeException(
              ...
              if (mappedValues.isEmpty) null
                ...
                throw new RuntimeException(
                  s"Too many mapped values obtained for ${colName}: ${mappedValues.mkString("|")}"
                )
              else {
                ...
                if (enumTypes.length != 1) {
                  throw new RuntimeException(
                    s"Found multiple enumTypes, expected only one: ${enumTypes}"
                  )
                } else {
          ...
          scribe.info(s"Output(${mapFieldNameToPFB(colName)}, ${if (mappedValue == null) "<null>"
          else mappedValue})")

          export.put(mapFieldNameToPFB(colName), mappedValue)
        })
